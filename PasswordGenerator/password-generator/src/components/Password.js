...
function Password() {
    ...
    const handleGeneratePassword = () =>{
        if(!includeLowerCase && !includeLowerCase && !includeSymbols && !includeNumbers){
            alert("To Generate Password yo must select atleast one checkbox")
        }else{
            let characterList = "";
            if(includeNumbers){
    ...
    const copyToClipboard = (password)=>{
        ...
        console.log(`copy to clipboard password is ${password}`)
    }
    ...
    const handleCopyPassword =(e)=>{
        console.log(`handleCopy Password value is ${password} & target action input is ${e.target.value}`);
        if(e.target.value === ""){
